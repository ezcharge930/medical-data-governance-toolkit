...
class DataReady:
    ...
    def fetch_norm_table(self, norm) -> pd.DataFrame:
        ds_id = norm['程序中数据集ID'].iloc[0]
        subjid = norm['subjid'].iloc[0]
        source_table = norm['来源表'].iloc[0]
        source_field = norm['来源变量'].tolist()
        # NOTE 这个地方用的loc,返回的是一个dataframe,所以where_condition是一个dateframe
        where_condition = norm.loc[norm['where语句']!='']
        where_str = ''
    ...
    def prepare_raw_data(self, config_file: str | None = None):
        # NOTE 获取dm集配置文件所在地
        ...
        norm_df.groupby(['程序中数据集ID'], as_index=False).apply(lambda x: self.fetch_norm_table(x))
...
class DM:
    ...
    def run_dm_ds(self):
        raw_data_store = self.dataready.prepare_raw_data(config_file= 'dm_rawdata_config.xlsx')
        self.core_task(raw_data_store)
        
    def core_task(self, raw_data_store):
        pass
